...
- **Rate limiting** (`express-rate-limit`, in-memory per worker): 100 req/min general, 10 req/min auth endpoints, 20 req/min upload, 30 req/min AI.
...
- **Graceful shutdown**: SIGTERM/SIGINT close the HTTP server with a 10s force-kill.
- **Health check**: `GET /health` — no auth, no rate limit, returns `{ status, uptime }`.
...
- `POST /api/ai/generate-content` uses Gemini for product title/description copy.
...
Copy `.env.example` → `.env`. Required vars: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `GOOGLE_GEMINI_API_KEY`.
