...
async function updateAdmin(req, res) {
    ...
    if (!existing)
        return res.status(404).json({ error: 'Admin not found' });
    if (email) {
        const { error: emailErr } = await supabase_1.supabase.auth.admin.updateUserById(id, { email });
        if (emailErr)
...
async function setAdminActive(req, res) {
    const { id } = req.params;
