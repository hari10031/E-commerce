...
async function getCategorySales(_req, res) {
    ...
    const [online, offline] = await Promise.all([
        supabase_1.supabase.from('order_items').select('quantity, unit_price, product:products(type)'),
        supabase_1.supabase.from('offline_sales').select('quantity, unit_price, product:products(type)'),
    ]);
    if (online.error)
        return res.status(500).json({ error: online.error.message });
    if (offline.error)
        return res.status(500).json({ error: offline.error.message });
    const grouped = {};
    const tally = (rows) => {
    ...
    tally(online.data);
    tally(offline.data);
    res.json(Object.entries(grouped)
