...
async function createRazorpayOrder(req, res) {
    ...
    const { error: bindErr } = await supabase_1.supabase
        .from('orders')
...
async function verifyPayment(req, res) {
    ...
    const { data: existing } = await supabase_1.supabase
        .from('orders')
        .select('id, status, total_amount, razorpay_order_id, coupon_applied, order_items(id, variant_id, quantity)')
        .eq('id', order_id)
    ...
    if (existing.razorpay_order_id && existing.razorpay_order_id !== razorpay_order_id) {
        return res.status(400).json({ error: 'Razorpay order does not match this order' });
    }
    const rzpOrder = await razorpay.orders.fetch(razorpay_order_id);
    const expectedPaise = Math.round(Number(existing.total_amount) * 100);
    ...
    const { data: order, error } = await supabase_1.supabase
        ...
        .single();
    if (error || !order) {
        return res.status(409).json({ error: 'Payment already processed or order state changed' });
    }
    for (const item of order.order_items) {
        const { error: stockErr } = await supabase_1.supabase.rpc('decrement_variant_stock', {
    ...
    if (order.coupon_applied) {
        await supabase_1.supabase.rpc('increment_coupon_usage', { code: order.coupon_applied });
    }
    ...
    queueService_1.notificationQueue.enqueue(() => (0, shipmentController_1.autoCreateShipment)(order.id));
