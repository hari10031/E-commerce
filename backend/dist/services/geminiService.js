...
function promptForType(productType) {
    const t = (productType || '').toLowerCase();
    if (t === 'jewellery' || t === 'jewelry' || t === 'gold')
        return (0, prompts_1.jewelleryPrompt)();
    return (0, prompts_1.sareePrompt)();
}
...
function buildVariantHint(color, category) {
    const parts = [];
    if (color?.trim()) {
        parts.push(`seller colour variant label: "${color.trim()}" — use only if it matches the dominant colour in the photo; otherwise follow the photo`);
    }
    if (category?.trim())
...
async function generateProductImage(input) {
    ...
    const response = await (0, fetchWithTimeout_1.withTimeout)(getClient().models.generateContent({
        model: IMAGE_MODEL,
        contents: [
            {
                role: 'user',
    ...
    }), GEMINI_TIMEOUT_MS, 'Gemini image generation');
...
{"title": "SEO-friendly title, max 80 characters", "description": "2-3 sentences covering fabric, occasion and colour appeal"}`;
    ...
    }), GEMINI_TIMEOUT_MS, 'Gemini content generation');
    const parts = response.candidates?.[0]?.content?.parts ?? [];
