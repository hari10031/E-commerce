...
export default function ProductWizardScreen({ route, navigation }) {
  ...
  const openWebHeroPreview = (color, label, asset) => {
    const previewUri = Platform.OS === 'web' && asset.file
      ? URL.createObjectURL(asset.file)
      : asset.uri;
    setPhotoPreview({ color, label, asset, previewUri });
  };
                ...
                >
                  {uploadedUri ? (
                    <View className="flex-1 relative">
                      <Image source={{ uri: uploadedUri }} className="w-full h-full" contentFit="cover"
