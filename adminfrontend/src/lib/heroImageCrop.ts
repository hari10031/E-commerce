/** Hero product photo: free crop region → 3:4 listing frame (matches storefront). */
...
export type NormCrop = { left: number; top: number; right: number; bottom: number };
...
/** Image bounds when `object-fit: contain` inside a box. */
...
export function clampCrop(rect: NormCrop, minSpan = 0.08): NormCrop {
...
export const FULL_CROP: NormCrop = { left: 0, top: 0, right: 1, bottom: 1 };

export function normToDisplay(rect: NormCrop, layout: ContainLayout) {
  return {
...
/**
 * Crop selection from image, then letterbox/cover into 3:4 (matches backend hero frame).
 */
export function renderCropTo34Canvas(img: HTMLImageElement, crop: NormCrop): HTMLCanvasElement {
...
export async function buildCroppedHeroFile(
  previewUri: string,
  crop: NormCrop,
  originalName: string
): Promise<File> {
